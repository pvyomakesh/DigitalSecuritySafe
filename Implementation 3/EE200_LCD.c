 ...
 * Author: SCHIANO
 *
 * Created on November 6, 2023, 1:13 AM
 */
...
void Init_LCD_DIO_Port (void) {
    ...
    TRISB = 0xC0F1; // set RB8 to RB13 for output, the rest for input
...
void Write_LCD_Nibble(int data, int cmd) {
 PORTB = (PORTB & 0x00F) | data << 8;     // place nibble at LCD inputs DB4 through DB7
